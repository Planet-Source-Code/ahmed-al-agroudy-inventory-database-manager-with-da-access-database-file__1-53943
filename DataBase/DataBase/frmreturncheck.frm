...
Begin VB.Form frmreturncheck 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Check Returner ÇáÔíßÇÊ ÇáãÑÊÌÚÉ "
   ClientHeight    =   5070
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10935
   Icon            =   "frmreturncheck.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5070
   ...
   Begin VB.Label Label3 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "Total Selected Amount is"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9
         Charset         =   178
   ...
   End
End
Attribute VB_Name = "frmreturncheck"
...
If MsgBox("Are You Sure You Want to exit With Out Save", vbYesNo, "Discard") = vbYes Then
    xtable = "delete from checkreturntemp where paymentno = '" & Trim(frmrecieptvou.txtreceiptnumber.Text) & "'"
    sqltable = True
    Dim recfind As New ADODB.Recordset
    myclass.GetTables recfind, CON1, xtable, constring, sqltable
    frmrecieptvou.cmbpaymenttype.ListIndex = 0
    frmrecieptvou.comsetmode.ListIndex = 0
    CON1.Close
    checknumberanddate = ""
    Unload Me
End If
...
sqltable = True
xtable = "delete checkreturntemp where paymentno = '" & Trim(frmrecieptvou.txtreceiptnumber.Text) & "'"
myclass.GetTables recdep, CON1, xtable, constring, sqltable
CON1.Close
'end clear

sqltable = True
xtable = "select * from checkreturntemp"
myclass.GetTables recdep, CON1, xtable, constring, sqltable
i = 1
If ListView1.ListItems.Count > 0 Then
    For i = 1 To ListView1.ListItems.Count
            ...
            If Trim(ListView1.ListItems(i).ListSubItems(1).Text) = "" Or Trim(ListView1.ListItems(i).ListSubItems(1).Text) = "No Date" Then
            Else
            ...
            'recdep!Paymode = Trim(ListView1.ListItems(i).ListSubItems(5).Text)
            recdep!paymentno = Trim(frmrecieptvou.txtreceiptnumber.Text) ' this is the payment no
            If ListView1.ListItems(i).Checked = True Then
                recdep!Selected = "1"
            End If
            recdep.Update
...
For i = 1 To ListView1.ListItems.Count
    ...
    End If
Next
frmrecieptvou.txtdebitamt.Text = ""
frmrecieptvou.txtdebitamt.Text = FormatNumber(total, 2)
frmrecieptvou.txtdebitamt.Enabled = False
frmrecieptvou.txtcheckdate.Enabled = False
frmrecieptvou.txtchecknumber.Text = ""
frmrecieptvou.txtchecknumber.Enabled = False
frmrecieptvou.checknumberanddate = checknumberanddate
Unload Me
End Sub
...
Dim recdep As New ADODB.Recordset
xtable = "select * from checkreturntemp where paymentno = '" & Trim(frmrecieptvou.txtreceiptnumber.Text) & "'"
myclass.GetTables recdep, CON1, xtable, constring, sqltable
i = 1
    If recdep.BOF = False Then
            While i <= recdep.RecordCount
                ListView1.ListItems.Add , , Trim(recdep!CheckNo)
                If IsNull(Trim(recdep!duedate)) = True Then
                    ListView1.ListItems(i).ListSubItems.Add , , " "
                Else
