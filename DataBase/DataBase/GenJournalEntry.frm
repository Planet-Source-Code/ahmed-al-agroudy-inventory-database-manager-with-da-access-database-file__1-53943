...
Begin VB.Form GenJournalEntry 
   Caption         =   "General Journal Entry   ≈œŒ«· ﬁÌÊœ «·ÌÊ„Ì…"
   ClientHeight    =   8490
   ClientLeft      =   75
   ClientTop       =   -1500
   ClientWidth     =   11880
...
End Sub
 ...
 Sub DeleteItems()
 xmsg = MsgBox("Delete TN «·€«¡  ‰" & Me.ListView1.SelectedItem & "?", vbQuestion + vbYesNo + vbDefaultButton2, "Please confirm „‰ ›÷·ﬂ «· «ﬂÌœ")
        If xmsg = vbYes Then
...
Me.caption = "General Journal Entry   ≈œŒ«· ﬁÌÊœ «·ÌÊ„Ì…  " & "//" & catName
DrCat = catName
End Sub
...
If KeyAscii = 13 Then
  If Me.Combo6 <> "" Or Me.Combo7 <> "" Then
    xmsg = MsgBox("Are Entries Okay? Â· «œŒ«·ﬂ ’ÕÌÕ ", vbQuestion + vbOKCancel, "Please confirm „‰ ›÷·ﬂ «· «ﬂÌœ")
    If xmsg = vbOK Then
        If Me.Combo16 = "" Then
...
If Me.Combo6 <> "" And Me.Combo7 = "" Then
    If Me.Combo6 = Me.Combo7 Then
     xmsg = MsgBox("You have entered same Account number in Debit Side" & vbCrLf & _
                    "«‰  „„ﬂ‰ «œŒ«· Õ”«»«  „‘»… »Ã«‰» «·„œÌ‰ ", vbInformation + vbOKOnly, "Message —”«·… ")
     Me.Combo6.SetFocus
     Exit Sub
    End If
End If
...
If RstBA.EOF = True Then
    RstBA.close
    xmsg = MsgBox("Account Number does not exist! «—ﬁ«„ «·Õ”«»«  ·« ÌÊÃœ", vbInformation + vbOKOnly, "Message —”«·…")
    Me.Combo6.SetFocus
    Exit Sub
Else
  Me.Combo12 = Trim(RstBA!accountnameeng) & "\" & Trim(RstBA!accountnamearab) ' '& " :—”Ì»·—ﬁ„ «·Õ”«»"
End If
...
RstBA.Open xKey, constring, adOpenDynamic, adLockOptimistic, adCmdText
If RstBA.EOF = True Then
  xmsg = MsgBox("Account Code does not exist «ﬂÊ«œ «·Õ”«»«  ·«  ÊÃœ", vbInformation + vbOKOnly, "Message —”«·… ")
    Me.Combo7.SetFocus
    Exit Sub
Else
  Me.Combo13 = RstBA!accountnameeng & "\" & RTrim(RstBA!accountnamearab)
...
If Me.Text2.Text <> "Edit" Then
  ...
  If Me.ListView1.ListItems.Count <> 0 Then
    If Trim(Me.SSTab1.caption) = "List" Then
      ...
      SendKeys "{Right}"
     End If
        msg = MsgBox("Do you want to keep unsaved entries ? Â· «‰   —Ìœ «·»Ì«‰«  ·« Õ›Ÿ", vbQuestion + vbYesNoCancel + vbDefaultButton1, "Please confirm „‰ ›÷·ﬂ «· «ﬂÌœ")
        If msg = vbYes Then
            pressno = 0
            rstTemp.close
            Unload Me
         ElseIf msg = vbNo Then
          xmsg = MsgBox("All your entries will be discarded if you Click Yes button Â·  —Ìœ «·€«¡ ﬂ· «·»Ì«‰«  «–« „ «ﬂœ «÷€ÿ „Ê«›ﬁ «”›· " & vbCrLf & _
                "Do you want to discard? Â·  —Ìœ «·€«¡ ", vbQuestion + vbYesNo, "Please confirm „‰ ›÷·ﬂ «· «ﬂÌœ")
          If xmsg = vbYes Then
...
If Me.ListView2.ListItems.Count <> 0 Then
    ...
    Dim MItem As ListItem
    mess = MsgBox("Do you want to continue? Â·  —Ìœ «·«” „—«—", vbQuestion + vbOKCancel + vbDefaultButton2, "Please confirm „‰ ›÷·ﬂ «· «ﬂÌœ")
    If mess = vbOK Then
...
Sub SaveEntries()
          ...
          If ccTotCr <> ccTotDr Then
            xmsg = MsgBox("Total Debit and Total Credit is not equal, Please equalized first before saving" & vbCrLf & _
                        "«Ã„«·Ì «·„Ì‰ Ê«Ã„«·Ì «·œ«∆‰ ·« Ì”«ÊÌ ,„‰ ›÷·ﬂ  «·Ìœ «Ê·« ﬁ»· «·Õ›Ÿ ", vbInformation + vbOKOnly, "Message —”«·…")
            Exit Sub
          End If
          xmsg = MsgBox(" Are you sure you want to save now?Â· «‰  „ «ﬂÌœ „‰ «·Õ›Ÿ «·«‰  ", vbQuestion + vbOKCancel, "Please Confirm „‰ ›÷·ﬂ «· «ﬂÌœ")
          If xmsg = vbOK Then
              Dim RstBA As ADODB.Recordset
              Set RstBA = New ADODB.Recordset
...
If Me.MaskEdBox1.Text = "" And Me.Combo6 <> "" Then
    xmsg = MsgBox("Please enter Transaction Amount „‰ ›÷·ﬂ «œŒ«· „Ã„Ê⁄ «·’›ﬁ« ", vbExclamation + vbOKOnly, "Message —”«·…")
    Exit Sub
End If
